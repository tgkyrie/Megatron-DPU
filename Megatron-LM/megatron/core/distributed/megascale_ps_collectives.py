...
def build_megascale_ps_group_name(scope: str, logical_name: str) -> str:
    if not logical_name:
    ...
    if scope == "dp":
        ...
        # All ranks in the same DP group should have the same MegaScalePS name.
        ...
        # CP rank varies inside the collective and must not be part of the MegaScalePS name.
        return f"dp.tp{tp_rank}.pp{pp_rank}.{logical_name}"
    if scope == "tp":
        # TP gradients are reduced across the TP group.
        # All ranks in the same TP group should have the same MegaScalePS name.
    ...
    raise ValueError(f"Unsupported MegaScalePS scope: {scope}")


def declare_and_cache_megascale_ps_group(name: str, expected_workers: int) -> None:
    ...
    if cached_workers != expected_workers:
        raise RuntimeError(
            f"MegaScalePS tensor name {name} was declared with inconsistent group size: "
...
def _declare_group_and_get_name(group, scope: str, logical_name: str) -> str:
    ...
    name = build_megascale_ps_group_name(scope, logical_name)
    declare_and_cache_megascale_ps_group(name, expected_workers)
    return name


def megascale_ps_allreduce(
    tensor,
    group,
    scope: str,
    logical_name: str,
...
def megascale_ps_allreduce_async_inplace(
    tensor,
    group,
    scope: str,
    logical_name: str,
...
def megascale_ps_allreduce_inplace(
    tensor,
    group,
    scope: str,
    logical_name: str,
...
def pre_declare_all_megascale_ps_groups(
    num_tp_layers: int,
    num_dp_buckets: int,
    ...
    """
    Pre-declare all MegaScalePS groups in a consistent order across all workers.
    ...
    Args:
        ...
        use_dpu_tp: Whether TP MegaScalePS is enabled
        use_dpu_dp: Whether DP MegaScalePS is enabled
    """
