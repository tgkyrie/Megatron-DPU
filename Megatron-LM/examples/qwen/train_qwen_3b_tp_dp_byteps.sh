...
#
# Notes:
# 1. DP gradient sync via --use-dpu-reduce
# 2. TP all-reduce via --use-dpu-tp-reduce
# 3. 默认打开 GDR；若只想先验证功能，可外部覆盖
#    DMLC_USE_GDR=0 或 BYTEPS_RDMA_NO_EXPAND=1
############################################
...
export NCCL_DEBUG_LEVEL=${NCCL_DEBUG_LEVEL:-WARN}
...
export BYTEPS_RDMA_REG_BACKOFF=${BYTEPS_RDMA_REG_BACKOFF:-1}
export BYTEPS_GDR_LEN_PROBE=${BYTEPS_GDR_LEN_PROBE:-0}
export BYTEPS_RDMA_NO_EXPAND=${BYTEPS_RDMA_NO_EXPAND:-0}
export BYTEPS_RDMA_MR_DEBUG=${BYTEPS_RDMA_MR_DEBUG:-0}
export BYTEPS_RDMA_EXACT_LOCAL_CACHE_MB=${BYTEPS_RDMA_EXACT_LOCAL_CACHE_MB:-4}
export BYTEPS_RDMA_MR_CACHE_LIMIT_MB=${BYTEPS_RDMA_MR_CACHE_LIMIT_MB:-512}
export BYTEPS_RDMA_HOST_MR_CACHE_LIMIT_MB=${BYTEPS_RDMA_HOST_MR_CACHE_LIMIT_MB:-1024}
export BYTEPS_GDR_FALLBACK=${BYTEPS_GDR_FALLBACK:-1}
export BYTEPS_GDR_FALLBACK_THRESHOLD_MB=${BYTEPS_GDR_FALLBACK_THRESHOLD_MB:-384}
...
if [[ "${TP_SIZE}" -eq 1 || "${DP_SIZE}" -eq 1 ]]; then
    echo "[warn] Current config does not exercise both TP and DP BytePS paths."
    echo "       WORLD_SIZE=${WORLD_SIZE} TP_SIZE=${TP_SIZE} DP_SIZE=${DP_SIZE}"
fi
...
echo "[gdr] DMLC_USE_GDR=${DMLC_USE_GDR} PART_BYTES=${BYTEPS_PARTITION_BYTES} RX_DEPTH=${BYTEPS_RDMA_RX_DEPTH} START_DEPTH=${BYTEPS_RDMA_START_DEPTH}"
echo "[gdr] REG_BACKOFF=${BYTEPS_RDMA_REG_BACKOFF} LEN_PROBE=${BYTEPS_GDR_LEN_PROBE} NO_EXPAND=${BYTEPS_RDMA_NO_EXPAND} MR_DEBUG=${BYTEPS_RDMA_MR_DEBUG}"
echo "[gdr] EXACT_CACHE_MB=${BYTEPS_RDMA_EXACT_LOCAL_CACHE_MB} MR_CACHE_MB=${BYTEPS_RDMA_MR_CACHE_LIMIT_MB} HOST_MR_CACHE_MB=${BYTEPS_RDMA_HOST_MR_CACHE_LIMIT_MB} FALLBACK=${BYTEPS_GDR_FALLBACK} FALLBACK_THRESHOLD_MB=${BYTEPS_GDR_FALLBACK_THRESHOLD_MB}"
