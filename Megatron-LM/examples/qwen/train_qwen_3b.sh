...
############################################
# DP MegaScalePS validation script
#
# Notes:
# 1. This script keeps the current default
#    values unchanged.
# 2. MegaScalePS is used for DP grad sync via
#    --use-dpu-reduce.
############################################
...
echo "[megascale_ps] NODE_RANK='"$NODE_RANK"' RANK=${RANK:-unset} LOCAL_RANK=$LOCAL_RANK CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
echo "[megascale_ps] MASTER_ADDR='"$MASTER_ADDR"' MASTER_PORT='"$MASTER_PORT"'"
echo "[megascale_ps] DMLC_NODE_HOST=$DMLC_NODE_HOST DMLC_PS_ROOT_URI=$DMLC_PS_ROOT_URI DMLC_PS_ROOT_PORT=$DMLC_PS_ROOT_PORT"
echo "[megascale_ps] DMLC_NUM_WORKER=$DMLC_NUM_WORKER MEGASCALE_PS_LOCAL_SIZE=$MEGASCALE_PS_LOCAL_SIZE MEGASCALE_PS_LOCAL_RANK=$MEGASCALE_PS_LOCAL_RANK DMLC_WORKER_ID=$DMLC_WORKER_ID"
