...
BATCH="$(date +%Y%m%d-%H%M%S)-vgg16-megascale_ps-8w8s"
...
clean_all() {
  for h in "${workers[@]}"; do
    remote_exec "$h" 'pids=$(pgrep -f "[w]orker.sh|[b]enchmark_megascale_ps|[b]pslaunch|[r]un-vgg16-megascale_ps" || true); if [ -n "$pids" ]; then kill -TERM $pids 2>/dev/null || true; fi' >/dev/null 2>&1 || true
  done
  for h in "${megascale_ps_nodes[@]}"; do
    remote_exec "$h" 'pids=$(pgrep -f "[s]cheduler.sh|[s]erver.sh|[b]enchmark_megascale_ps|[b]pslaunch|[r]un-vgg16-megascale_ps" || true); if [ -n "$pids" ]; then kill -TERM $pids 2>/dev/null || true; fi' >/dev/null 2>&1 || true
  done
  sleep 3
}

start_megascale_ps_roles() {
  local run="$1" envs
  envs="$(topology_env)"
...
cd /tmp
timeout 900s bash /usr/local/megascale_ps/sh/worker.sh > /tmp/${run}-worker.log 2>&1
...
run="run-vgg16-megascale_ps-8w8s-$BATCH"
clean_all
echo "[$(date '+%H:%M:%S')] START MegaScalePS VGG16 run=$run"
start_megascale_ps_roles "$run" || {
  echo "start_megascale_ps_roles failed"
  exit 1
}
start_workers "$run" || {
...
echo -e "MegaScalePS\t8\t8\tvgg16\t$final_status\t$per_mean\t$per_conf\t$total_mean\t$total_conf\t$rank0_log\t$final_status" >> "$TSV"

{
  echo "# VGG16 MegaScalePS 8-worker/8-server benchmark"
  echo
  echo "Run: $BATCH"
  echo "Date: $(date -Is)"
  echo
  ...
  echo "- Role scripts: /usr/local/megascale_ps/sh/scheduler.sh, server.sh, worker.sh"
  echo "- Runner exports: DMLC_NUM_WORKER, DMLC_NUM_SERVER, WORKER_ID only"
  echo "- Benchmark defaults come from worker.sh: benchmark_megascale_ps.py --model vgg16 --num-iters 10"
  echo
  echo "## Result"
  echo
  echo "| Metric | Result |"
