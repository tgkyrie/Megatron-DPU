...
BATCH="$(date +%Y%m%d-%H%M%S)-megascale_ps-payload-scaling-matrix"
...
scales=(${SCALES:-2 4 8})
systems=(${SYSTEMS:-megascale_ps native})
sizes=(${SIZES:-1})
...
run_payload() {
  ...
  if [[ -n "$megascale_ps_extra_env" ]]; then
...
cd /tmp
timeout ${timeout_s}s bpslaunch python3 /usr/local/megascale_ps/example/pytorch/pushpull_bench.py --size-mb $size --iters 40 --warmup 5 > /tmp/${run}-worker.log 2>&1
...
for scale in "${scales[@]}"; do
  ...
  for size in "${sizes[@]}"; do
    for sys in "${systems[@]}"; do
      case "$sys" in
        megascale_ps) run_payload "MegaScale-PS" "$scale" "$megascale_ps_container" "$size" ;;
        native) run_payload "Native-MegaScalePS" "$scale" "$native_container" "$size" ;;
...
{
  echo "# MegaScalePS payload scaling matrix"
  echo
  echo "Run: $BATCH"
  echo "Date: $(date -Is)"
  echo
  echo "This is a push-pull communication microbenchmark. MegaScalePS-style systems use equal worker and server counts on disjoint machines."
  echo
