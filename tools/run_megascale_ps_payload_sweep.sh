...
BATCH="$(date +%Y%m%d-%H%M%S)-megascale_ps-payload-sweep-8w8s"
...
clean_all() {
  ...
  for h in "${megascale_ps_nodes[@]}"; do
    remote_exec "$h" 'pids=$(pgrep -f "[s]cheduler.sh|[s]erver.sh|[p]ushpull_bench|[b]pslaunch|[b]enchmark_megascale_ps" || true); if [ -n "$pids" ]; then kill -TERM $pids 2>/dev/null || true; fi' >/dev/null 2>&1 || true
  done
  sleep 3
}
...
export MEGASCALE_PS_LOCAL_SIZE=1
export MEGASCALE_PS_LOCAL_RANK=0
export CUDA_VISIBLE_DEVICES=1
export MEGASCALE_PS_PARTITION_BYTES=4194304
export MEGASCALE_PS_RDMA_RX_DEPTH=512
export MEGASCALE_PS_RDMA_START_DEPTH=32
cd /usr/local/megascale_ps
timeout ${payload_timeout}s bpslaunch python3 /usr/local/megascale_ps/example/pytorch/pushpull_bench.py --size-mb $size --iters 40 --warmup 5 > /tmp/${run}-worker.log 2>&1
...
run_one() {
  local size="$1" label
  label="${size//./p}"
  local run="megascale_ps-payload-${label}mb-$BATCH"
  clean_all
  echo "[$(date '+%H:%M:%S')] START payload size=${size}MB"
  start_megascale_ps "$run" || return 1
...
{
  echo "# MegaScalePS payload sweep"
  echo
  echo "Batch: $BATCH"
  echo "Date: $(date -Is)"
  echo
  echo "This is a MegaScalePS push-pull communication microbenchmark, not a Megatron end-to-end training measurement."
  echo
  echo "- Scheduler: gpu01"
  echo "- Workers: ${workers[*]}"
  echo "- Servers: ${servers[*]}"
  echo "- Worker command: bpslaunch python3 /usr/local/megascale_ps/example/pytorch/pushpull_bench.py --iters 40 --warmup 5"
