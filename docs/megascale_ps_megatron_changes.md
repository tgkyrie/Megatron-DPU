# Megatron + MegaScalePS 主要改动整理（tgkyrie 相关）

本文件整理 tgkyrie 的关键提交、Megatron 后端通信替换为 MegaScalePS 的实现点，以及 MegaScalePS push/pull 相关优化，并给出核心代码位置与行号。

## 一、关键提交时间线（tgkyrie）

| 日期 | 提交 | 主题 | 说明 |
|---|---|---|---|
| 2025-11-24 | `a39bfe7` | [Init] | 仓库初始化结构（Megatron-LM/MegaScalePS 引入）。 |
| 2025-11-24 | `e76c4d3` | [INIT] | 大量导入 Megatron-LM 与 MegaScalePS 代码。 |
| 2025-11-25 | `d72d072` | [INIT] | 首次把 Megatron 的 DP 梯度同步改成 MegaScalePS。 |
...
| 2026-01-27 | `e23c5f4` | [OPT] Optimize MegaScalePS Bandwidth | RDMA 双通道 + push/pull 多线程等带宽优化。 |
| 2026-01-27 | `67ff54e` | [FIX] fix a MegatronLM bug | 非主线功能修复。 |

## 二、工作目标与落地点

### 1) 将 Megatron 后端通信改为 MegaScalePS
...
- `Megatron-LM/megatron/core/distributed/param_and_grad_buffer.py:381-417`
  - 在不使用分布式优化器时，改用 MegaScalePS `push_pull` 完成 DP 梯度同步。
...
### 2) 优化 MegaScalePS push/pull 逻辑

#### 2.1 RDMA 双通道（控制/数据分离）

**新增 data-plane endpoint**
- 连接与 endpoint 管理：`megascale_ps/3rdparty/ps-lite/src/rdma_van.h:177-260`
- 发送逻辑（push/pull 走 data endpoint）：`megascale_ps/3rdparty/ps-lite/src/rdma_van.h:384-461`
- data_endpoints 容器：`megascale_ps/3rdparty/ps-lite/src/rdma_van.h:928-934`

**协议字段与结构支持**
- `RequestContext.isDataPlane`：`megascale_ps/3rdparty/ps-lite/src/rdma_utils.h:171-175`
- `Endpoint.isDataPlane`：`megascale_ps/3rdparty/ps-lite/src/rdma_transport.h:28-68`

**对比原始 MegaScalePS**
- 原始 MegaScalePS 只有单一 RDMA endpoint，控制与数据复用。

#### 2.2 Push/Pull 多线程

- 新增 `MEGASCALE_PS_PUSH_THREAD`：`megascale_ps/megascale_ps/common/global.cc:120-162`
- 根据线程数启动多个 PushLoop/PullLoop：`megascale_ps/megascale_ps/common/operations.cc:47-71`

**对比原始 MegaScalePS**
- 原始 MegaScalePS 默认只启动单线程 push/pull。

#### 2.3 Key Hash 策略调整（负载分布）

- 默认 hash 改为 `raw`：`megascale_ps/megascale_ps/common/global.cc:154-163`
- `raw` 分配逻辑：`megascale_ps/megascale_ps/common/global.cc:630-676`
...
- PUSH 队列 debug：`megascale_ps/megascale_ps/common/scheduled_queue.cc:101-105`
- 追加 `get_pushpull_speed` 接口：`megascale_ps/megascale_ps/torch/ops.py:37-46`
- 新增 push/pull benchmark：`megascale_ps/example/pytorch/pushpull_bench.py`

---

## 三、对比“原有 MegaScalePS”的核心差异总结

1. **RDMA 双通道**：新增 data-plane endpoint，push/pull 数据走 data endpoint（提升带宽）。
2. **多线程 Push/Pull**：通过 `MEGASCALE_PS_PUSH_THREAD` 并发 push/pull。
...
- **目标 1：Megatron 后端通信换为 MegaScalePS**
  - 配置开关 + CLI 参数 + 初始化 `bps.init()` + DP 梯度同步替换（见第 2 节）。

- **目标 2：优化 MegaScalePS push/pull 逻辑**
...
1. **Megatron 的 DP 同步替换为 MegaScalePS**
   - 目的：在多机、多卡场景下，用 MegaScalePS 的 PS/调度机制替代传统 NCCL allreduce，提升跨机带宽利用与可调度性。
   - 做法：在 DP 梯度同步入口处切换到 `bps.push_pull`，通过 `--use-dpu-reduce` 控制开关，统一在初始化阶段 `bps.init()`，避免每个训练脚本各自初始化。

2. **MegaScalePS push/pull 侧的带宽优化**
...
- **性能调参空间扩大**：
  通过 `MEGASCALE_PS_PUSH_THREAD`、hash 策略等参数可针对不同网络/拓扑调优。
...
### 7.1 d72d072：Megatron DP 同步由 NCCL allreduce -> MegaScalePS push/pull

**代码作用**
在 DP 梯度同步阶段，用 MegaScalePS `push_pull` 替代 NCCL `all_reduce`。

**为什么这样做**
- MegaScalePS 更适合跨机调度与带宽优化；
- 可以通过开关做 A/B 测试与回退。

**流程（摘要）**
1. 进入梯度归约阶段；
2. 若满足 `use_dpu_reduce`，走 MegaScalePS 分支；
...
**修改后（MegaScalePS push/pull）**
```python
# Megatron-LM/megatron/core/distributed/param_and_grad_buffer.py
# 注：早期字段名为 use_megascale_ps_for_grad_sync，后续改为 use_dpu_reduce
...
**代码作用**
提供统一开关与初始化位置，控制是否启用 MegaScalePS。
...
### 7.3 a90a8ca：MegaScalePS 梯度同步 in-place
...
**修改后（in-place）**
```python
bps.push_pull_inplace(bucket.grad_data, average=megascale_ps_average, name=name)
```
...
**修改前（单 endpoint）**
```cpp
// megascale_ps/3rdparty/ps-lite/src/rdma_van.h
...
**流程（摘要）**
1. 读取 `MEGASCALE_PS_PUSH_THREAD`；
2. 启动多个 PushLoop/PullLoop；
3. 默认 hash 改为 `raw`。

**修改前（单线程 + djb2）**
```cpp
// megascale_ps/megascale_ps/common/global.cc
_start_step = getenv("MEGASCALE_PS_TRACE_START_STEP") ? ... : _start_step;
_end_step = getenv("MEGASCALE_PS_TRACE_END_STEP") ? ... : _end_step;
...
_is_distributed_job = (_num_worker > 1) ? true : _is_distributed_job;
_hash_knob = std::string(getenv("MEGASCALE_PS_KEY_HASH_FN") ? ... : "djb2");
```
...
// megascale_ps/megascale_ps/common/global.cc
_start_step = getenv("MEGASCALE_PS_TRACE_START_STEP") ? ... : _start_step;
_push_thread = getenv("MEGASCALE_PS_PUSH_THREAD") ? atoi(...) : 1;
_end_step = getenv("MEGASCALE_PS_TRACE_END_STEP") ? ... : _end_step;
...
_is_distributed_job = (_num_worker > 0) ? true : _is_distributed_job;
_hash_knob = std::string(getenv("MEGASCALE_PS_KEY_HASH_FN") ? ... : "raw");
```

**修改前（单线程 Push/Pull）**
```cpp
// megascale_ps/megascale_ps/common/operations.cc
if (MegaScalePSGlobal::IsRootDevice()) {
...
// megascale_ps/megascale_ps/common/operations.cc
if (MegaScalePSGlobal::IsRootDevice()) {
  for (int i = 0; i < MegaScalePSGlobal::GetPushThread(); i++) {
    func.push_back(PullLoop);
  }
  func.push_back(DecompressLoop);
}
...
## 八、分支差异（megascale_ps 分支 vs 其他分支）

> 说明：本仓库当前没有名为 `megascale_ps` 的分支；常用的是 `wpb_megascale_ps` 与 `master`。若你指的是其他远端分支（如 `origin/hc_bps`），请明确分支名。

### 7.1 `master` vs `wpb_megascale_ps`

- 当前 **`master` 与 `wpb_megascale_ps` 指向同一提交**（无差异）。
...
megascale_ps/3rdparty/ps-lite/src/rdma_van.h             |  174 +-
megascale_ps/megascale_ps/common/global.cc                     |   58 +-
megascale_ps/megascale_ps/common/operations.cc                 |    8 +-
megascale_ps/example/train_qwen_3b.sh                    |  236 -
megascale_ps/sh/server.sh                                |   62 +-
megascale_ps/sh/scheduler.sh                             |   58 +-
...
```
