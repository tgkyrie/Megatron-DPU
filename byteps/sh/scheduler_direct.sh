...
# Direct per-GPU process scheduler. This matches worker_direct_gpu.sh with
# BYTEPS_LOCAL_SIZE=1 and does not imply legacy GDR.
...
export BYTEPS_RDMA_RX_DEPTH=${BYTEPS_RDMA_RX_DEPTH:-64}
export BYTEPS_RDMA_START_DEPTH=${BYTEPS_RDMA_START_DEPTH:-8}
...
echo "[direct-scheduler] NUMA_NODE=${NUMA_NODE} CPU_LIST='${CPU_LIST}' "\
...
cleanup() {
  ...
  STOPPED=1
  echo "[direct-scheduler] stopping"
