...
#
# ResNet-50 model training using Keras and MegaScalePS.
#
...
import megascale_ps.keras as bps
import os
...
# initialize MegaScalePS
bps.init()

# MegaScalePS: pin GPU to be used to process local rank (one GPU per process)
...
# MegaScalePS: broadcast resume_from_epoch from rank 0 (which will have
# checkpoints) to other ranks.
resume_from_epoch = bps.broadcast(resume_from_epoch, 0, name='resume_from_epoch')

# MegaScalePS: print logs on the first worker.
...
# MegaScalePS: (optional) compression algorithm.
compression = bps.Compression.fp16 if args.fp16_pushpull else bps.Compression.none

# Restore from a previous checkpoint, if initial_epoch is specified.
# MegaScalePS: restore on the first worker which will broadcast both model and optimizer weights
# to other workers.
if resume_from_epoch > 0 and bps.rank() == 0:
    ...
    # MegaScalePS: adjust learning rate based on number of GPUs.
    opt = keras.optimizers.SGD(lr=args.base_lr * bps.size(),
                               momentum=args.momentum)

    # MegaScalePS: add MegaScalePS Distributed Optimizer.
...
callbacks = [
    # MegaScalePS: broadcast initial variable states from rank 0 to all other processes.
    # This is necessary to ensure consistent initialization of all workers when
    # training is started with random weights or restored from a checkpoint.
    bps.callbacks.BroadcastGlobalVariablesCallback(0),

    # MegaScalePS: average metrics among workers at the end of every epoch.
    #
    # Note: This callback must be in the list before the ReduceLROnPlateau,
    # TensorBoard, or other metrics-based callbacks.
    bps.callbacks.MetricAverageCallback(),

    # MegaScalePS: using `lr = 1.0 * bps.size()` from the very beginning leads to worse final
    ...
    # MegaScalePS: after the warmup reduce learning rate by 10 on the 30th, 60th and 80th epochs.
...
# MegaScalePS: save checkpoints only on the first worker to prevent other workers from corrupting them.
if bps.rank() == 0:
