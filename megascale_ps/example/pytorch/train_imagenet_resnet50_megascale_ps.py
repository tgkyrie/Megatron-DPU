...
if args.cuda:
    # MegaScalePS: pin GPU to local rank.
...
# MegaScalePS: write TensorBoard logs on first worker.
...
# MegaScalePS: use DistributedSampler to partition data among workers. Manually specify
...
# MegaScalePS: scale learning rate by the number of GPUs.
...
# MegaScalePS: wrap optimizer with DistributedOptimizer.
...
# MegaScalePS: restore on the first worker which will broadcast weights to other workers.
...
# MegaScalePS: broadcast parameters & optimizer state.
...
# MegaScalePS: using `lr = base_lr * bps.size()` from the very beginning leads to worse final
...
# MegaScalePS: average metrics from distributed training.
class Metric(object):
