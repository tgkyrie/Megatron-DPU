...
import megascale_ps.mxnet as bps
import mxnet as mx
import numpy as np
...
class MXTest(unittest.TestCase, metaclass=MetaTest):
    """
    Tests for ops in megascale_ps.mxnet.
    """
    ...
    def test_megascale_ps_trainer_param_order(self):
        ...
        print('test_megascale_ps_trainer_param_order passed')

    def test_megascale_ps_push_pull(self):
        """Test that the megascale_ps_push_pull correctly sums 1D, 2D, 3D tensors."""
        ...
        for dtype, dim in itertools.product(dtypes, dims):
            ...
            bps.megascale_ps_declare_tensor("tensor_" + str(count))
            bps.megascale_ps_push_pull(tensor, name="tensor_"+str(count))
            tensor.wait_to_read()
            output = tensor.asnumpy()
            assert np.allclose(input, output)
            count += 1

        print('test_megascale_ps_push_pull passed')

    def test_megascale_ps_push_pull_inplace(self):
        """Test that the megascale_ps_push_pull correctly sums 1D, 2D, 3D tensors."""
        size = bps.size()
        dtypes = ['float16', 'float32', 'float64']
        dims = [1, 2, 3]
        ...
        for dtype, dim in itertools.product(dtypes, dims):
            ...
            assert max_difference <= threshold, 'bps.megascale_ps_push_pull produces \
                                                 incorrect results for self'

        print('test_megascale_ps_push_pull_inplace passed')
