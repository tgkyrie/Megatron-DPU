...
def allocate_cpu(local_size):
    cpu_mt = os.getenv("MEGASCALE_PS_MULTITHREADED_CPU", "1").lower() in ["1", "true"]
    def get_numa_info():
        """
    ...
    def _get_quota(nodes, local_size):
        ...
        default_quota = int(os.getenv("MEGASCALE_PS_NUMA_DEFAULT_QUOTA", default_quota))
        ...
        if int(os.getenv("MEGASCALE_PS_NUMA_ROOT_QUOTA", 0)):
            root_quota = int(os.getenv("MEGASCALE_PS_NUMA_ROOT_QUOTA", 0))
    ...
    cpu_blacklist = os.getenv("MEGASCALE_PS_CPU_BLACKLIST", "-1")
...
def worker(local_rank, local_size, command, allocation=None):
    ...
    my_env["MEGASCALE_PS_LOCAL_RANK"] = str(local_rank)
    my_env["MEGASCALE_PS_LOCAL_SIZE"] = str(local_size)
    if int(os.getenv("MEGASCALE_PS_ENABLE_GDB", 0)):
    ...
    if os.environ.get("MEGASCALE_PS_TRACE_ON", "") == "1":
        print("\n!!!Enable profiling for WORKER_ID: %s and local_rank: %d!!!" %
              (os.environ.get("DMLC_WORKER_ID"), local_rank))
        print("MEGASCALE_PS_TRACE_START_STEP: %s\tMEGASCALE_PS_TRACE_END_STEP: %s\t MEGASCALE_PS_TRACE_DIR: %s" % (os.environ.get(
            "MEGASCALE_PS_TRACE_START_STEP", ""), os.environ.get("MEGASCALE_PS_TRACE_END_STEP", ""), os.environ.get("MEGASCALE_PS_TRACE_DIR", "")))
        print("Command: %s\n" % command)
        sys.stdout.flush()
        trace_path = os.path.join(os.environ.get(
            "MEGASCALE_PS_TRACE_DIR", "."), str(local_rank))
...
def get_local_size():
    explicit_local_size = os.getenv("MEGASCALE_PS_LOCAL_SIZE", "").strip()
...
def parse_num_range(core_list):
    ...
    # core assignment for the corresponding megascale_ps worker.
...
def join_threads(threads):
    ...
    while count < num:
        with cv:
            ...
            done_threads.pop()
        threads[idx].join()
        print("MegaScalePS launcher: joined local rank ", idx)
        count += 1

def launch_bps():
    print("MegaScalePS launching " + os.environ["DMLC_ROLE"])
    sys.stdout.flush()
    check_env()
    ...
    if os.environ["DMLC_ROLE"] == "worker":
        ...
        bind_to_cores = os.getenv("MEGASCALE_PS_NUMA_ON", "1") == "1"
        if bind_to_cores:
            user_override = os.getenv("MEGASCALE_PS_VISIBLE_CPU_CORES", "").strip()
            if user_override:
    ...
    elif os.environ.get("MEGASCALE_PS_FORCE_DISTRIBUTED", "") == "1" or \
         int(os.environ.get("DMLC_NUM_WORKER", "1")) > 1:
        command = "python3 -c 'import megascale_ps.server'"
        if int(os.getenv("MEGASCALE_PS_ENABLE_GDB", 0)):
            command = "gdb -ex 'run' -ex 'bt' -batch --args " + command
        print("Command: %s\n" % command, flush=True)
        my_env = os.environ.copy()
