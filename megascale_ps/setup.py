...
server_lib = Extension('megascale_ps.server.c_lib', [])
tensorflow_lib = Extension('megascale_ps.tensorflow.c_lib', [])
mxnet_lib = Extension('megascale_ps.mxnet.c_lib', [])
pytorch_lib = Extension('megascale_ps.torch.c_lib', [])

# Package meta-data.
NAME = 'megascale_ps'
DESCRIPTION = 'A high-performance cross-framework Parameter Server for Deep Learning'
URL = 'https://github.com/bytedance/megascale_ps'
...
def get_mpi_flags():
    show_command = os.environ.get('MEGASCALE_PS_MPICXX_SHOW', 'mpicxx -show')
    try:
        ...
        raise DistutilsPlatformError(
            ...
            'please specify it with the MEGASCALE_PS_MPICXX_SHOW environment variable.\n\n'
...
def get_link_flags(build_ext):
    last_err = None
    libtool_flags = ['-Wl,-exported_symbols_list,megascale_ps.exp']
    ld_flags = ['-Wl,--version-script=megascale_ps.lds', '-fopenmp']
    flags_to_try = []
    if sys.platform == 'darwin':
...
def use_ucx():
    megascale_ps_with_ucx = int(os.environ.get('MEGASCALE_PS_WITH_UCX', 0))
    return megascale_ps_with_ucx


def get_cuda_home():
    return os.environ.get('MEGASCALE_PS_CUDA_HOME', '/usr/local/cuda')
...
def use_ucx_cuda():
    forced = os.environ.get('MEGASCALE_PS_UCX_WITH_CUDA')
    if forced is not None:
        return int(forced)
    if int(os.environ.get('MEGASCALE_PS_FORCE_NO_CUDA', 0)):
        return 0
    if int(os.environ.get('MEGASCALE_PS_SERVER_ONLY', 0)):
        return 0
...
def use_pslite_cuda():
    forced = os.environ.get('MEGASCALE_PS_PSLITE_WITH_CUDA')
    if forced is not None:
        return int(forced)
...
def server_only():
    return int(os.environ.get('MEGASCALE_PS_SERVER_ONLY', 0))


def with_pre_setup():
    return int(os.environ.get('MEGASCALE_PS_WITHOUT_PRESETUP', 0)) == 0

def with_tensorflow():
    return int(os.environ.get('MEGASCALE_PS_WITH_TENSORFLOW', 0))

def without_tensorflow():
    return int(os.environ.get('MEGASCALE_PS_WITHOUT_TENSORFLOW', 0))

def with_pytorch():
    return int(os.environ.get('MEGASCALE_PS_WITH_PYTORCH', 0))

def without_pytorch():
    return int(os.environ.get('MEGASCALE_PS_WITHOUT_PYTORCH', 0))

def should_build_ucx():
    has_prebuilt_ucx = os.environ.get('MEGASCALE_PS_UCX_HOME', '')
    return use_ucx() and not has_prebuilt_ucx

ucx_default_home = '/usr/local'
def get_ucx_prefix():
    """ specify where to install ucx """
    ucx_prefix = os.getenv('MEGASCALE_PS_UCX_PREFIX', ucx_default_home)
    return ucx_prefix
...
def get_ucx_home():
    ...
    return os.environ.get('MEGASCALE_PS_UCX_HOME', ucx_default_home)
...
def get_common_options(build_ext):
    ...
    SOURCES = ['megascale_ps/common/common.cc',
               'megascale_ps/common/operations.cc',
               'megascale_ps/common/core_loops.cc',
               'megascale_ps/common/cuda_ready_event.cc',
               'megascale_ps/common/global.cc',
               'megascale_ps/common/logging.cc',
               'megascale_ps/common/communicator.cc',
               'megascale_ps/common/scheduled_queue.cc',
               'megascale_ps/common/ready_table.cc',
               'megascale_ps/common/shared_memory.cc',
               'megascale_ps/common/nccl_manager.cc',
               'megascale_ps/common/cpu_reducer.cc'] + [
               'megascale_ps/common/compressor/compressor_registry.cc',
               'megascale_ps/common/compressor/error_feedback.cc',
               'megascale_ps/common/compressor/momentum.cc',
               'megascale_ps/common/compressor/impl/dithering.cc',
               'megascale_ps/common/compressor/impl/onebit.cc',
               'megascale_ps/common/compressor/impl/randomk.cc',
               'megascale_ps/common/compressor/impl/topk.cc',
               'megascale_ps/common/compressor/impl/vanilla_error_feedback.cc',
               'megascale_ps/common/compressor/impl/nesterov_momentum.cc']
    if "MEGASCALE_PS_USE_MPI" in os.environ and os.environ["MEGASCALE_PS_USE_MPI"] == "1":
        mpi_flags = get_mpi_flags()
        COMPILE_FLAGS = cpp_flags + \
            shlex.split(mpi_flags) + ["-DMEGASCALE_PS_USE_MPI"]
...
def build_server(build_ext, options):
    ...
    server_lib.sources = ['megascale_ps/server/server.cc',
                          'megascale_ps/common/cpu_reducer.cc',
                          'megascale_ps/common/logging.cc',
                          'megascale_ps/common/common.cc'] + [
                          'megascale_ps/common/compressor/compressor_registry.cc',
                          'megascale_ps/common/compressor/error_feedback.cc',
                          'megascale_ps/common/compressor/impl/dithering.cc',
                          'megascale_ps/common/compressor/impl/onebit.cc',
    ...
    try:
        ...
        if LooseVersion(tf.__version__) < LooseVersion('1.1.0'):
            raise DistutilsPlatformError(
                ...
                'MegaScalePS requires tensorflow>=1.1.0' % tf.__version__)
    except ImportError:
        raise DistutilsPlatformError(
        ...
        raise DistutilsPlatformError(
            'Your TensorFlow version is outdated. MegaScalePS requires tensorflow>=1.1.0')
...
def build_tf_extension(build_ext, options):
    ...
    tensorflow_lib.sources = options['SOURCES'] + \
        ['megascale_ps/tensorflow/ops.cc']
    ...
    try:
        ...
        if mx.__version__ < '1.4.0':
            raise DistutilsPlatformError(
                ...
                'MegaScalePS requires mxnet>=1.4.0' % mx.__version__)
    except ImportError:
        raise DistutilsPlatformError(
            'import mxnet failed, is it installed?\n\n%s' % traceback.format_exc())
    except AttributeError:
        raise DistutilsPlatformError(
            'Your MXNet version is outdated. MegaScalePS requires mxnet>=1.4.0')
...
def get_mx_include_dirs():
    try:
        ...
        tmp_mxnet_dir = os.getenv(
            "MEGASCALE_PS_SERVER_MXNET_PATH", "/root/mxnet15-rdma")
...
def get_cuda_dirs(build_ext, cpp_flags):
    ...
    cuda_home = os.environ.get('MEGASCALE_PS_CUDA_HOME')
    if cuda_home:
        cuda_include_dirs += ['%s/include' % cuda_home]
        cuda_lib_dirs += ['%s/lib' % cuda_home, '%s/lib64' % cuda_home]

    cuda_include = os.environ.get('MEGASCALE_PS_CUDA_INCLUDE')
    if cuda_include:
        cuda_include_dirs += [cuda_include]

    cuda_lib = os.environ.get('MEGASCALE_PS_CUDA_LIB')
    if cuda_lib:
    ...
    try:
        ...
        raise DistutilsPlatformError(
            ...
            'Please specify correct CUDA location with the MEGASCALE_PS_CUDA_HOME '
            'environment variable or combination of MEGASCALE_PS_CUDA_INCLUDE and '
            'MEGASCALE_PS_CUDA_LIB environment variables.\n\n'
            'MEGASCALE_PS_CUDA_HOME - path where CUDA include and lib directories can be found\n'
            'MEGASCALE_PS_CUDA_INCLUDE - path to CUDA include directory\n'
            'MEGASCALE_PS_CUDA_LIB - path to CUDA lib directory')
...
def get_nccl_vals():
    ...
    nccl_home = os.environ.get('MEGASCALE_PS_NCCL_HOME', '/usr/local/nccl')
    if nccl_home:
        nccl_include_dirs += ['%s/include' % nccl_home]
        nccl_lib_dirs += ['%s/lib' % nccl_home, '%s/lib64' % nccl_home]

    nccl_link_mode = os.environ.get('MEGASCALE_PS_NCCL_LINK', 'SHARED')
...
def build_mx_extension(build_ext, options):
    ...
    if not mx_have_cuda and macro_have_cuda:
        raise DistutilsPlatformError(
            'MegaScalePS build with GPU support was requested, but this MXNet '
    ...
    mxnet_lib.sources = options['SOURCES'] + \
        ['megascale_ps/mxnet/ops.cc',
         'megascale_ps/mxnet/ready_event.cc',
         'megascale_ps/mxnet/tensor_util.cc',
         'megascale_ps/mxnet/cuda_util.cc',
         'megascale_ps/mxnet/adapter.cc']
    ...
    try:
        ...
        if torch.__version__ < '1.0.1':
            raise DistutilsPlatformError(
                ...
                'MegaScalePS requires torch>=1.0.1' % torch.__version__)
    except ImportError:
...
def build_torch_extension(build_ext, options, torch_version):
    ...
    if not have_cuda and check_macro(options['MACROS'], 'HAVE_CUDA'):
        raise DistutilsPlatformError(
            'megascale_ps build with GPU support was requested, but this PyTorch '
            'installation does not support CUDA.')
    ...
    ext = TorchExtension(pytorch_lib.name,
                         ...
                         sources=options['SOURCES'] + ['megascale_ps/torch/ops.cc',
                                                       'megascale_ps/torch/ready_event.cc',
                                                       'megascale_ps/torch/cuda_util.cc',
                                                       'megascale_ps/torch/adapter.cc',
                                                       'megascale_ps/torch/handle_manager.cc'],
                         extra_compile_args=pytorch_compile_flags,
...
def build_ucx():
    ucx_tarball_path = os.getenv("MEGASCALE_PS_UCX_TARBALL_PATH", "")
    ...
    if ucx_tarball_path:
        ...
        ucx_version = os.getenv("MEGASCALE_PS_UCX_VERSION", "1.20.0")
...
class custom_build_ext(build_ext):
    def build_extensions(self):
        ...
        if server_only():
            print('INFO: MEGASCALE_PS_SERVER_ONLY=1; server built; skip TF/Torch/MXNet plugins')
            return
        ...
        if not without_pytorch():
            try:
                ...
                if not with_pytorch():
                    ...
                    raise
        if not int(os.environ.get('MEGASCALE_PS_WITHOUT_MXNET', 0)):
            # fix "libcuda.so.1 not found" issue
            cuda_home = os.environ.get('MEGASCALE_PS_CUDA_HOME', '/usr/local/cuda')
            ...
            try:
                ...
                if not int(os.environ.get('MEGASCALE_PS_WITH_MXNET', 0)):
...
if not os.path.exists('3rdparty/ps-lite/src'):
    msg = "Missing ./3rdparty/ps-lite, ps-lite is required to build MegaScalePS."
...
if int(os.environ.get('MEGASCALE_PS_WITHOUT_MXNET', 0)):
