...
### **MegaScalePS has a better performance because of some data path details, e.g., less copying?**

Not really. MegaScalePS uses share memory in a similar way as NCCL, and MegaScalePS copies the data for as many times as NCCL. In addition, both MegaScalePS and NCCL use tensor partition/segmentation internally, which hides most of the copying delay.

I would consider MegaScalePS and NCCL are both close to their theoretical optimal based on their communication patterns.

### **MegaScalePS has a better performance because it has a hierarchical strategy, i.e., local reduce followed by inter-machine transfers?**
...
MegaScalePS does not have this problem. It can fully utilize bi-direction network bandwidth. The key ideas are tensor partitioning and pipelining. For example, you have a 100MB tensor to be pushed and pulled. Inside MegaScalePS, we will partition the tensor into small pieces. After pushing the first piece, we will start pulling the first piece. At the same time, we will start pushing the second piece. And so on. For most of the time except the first piece and the last piece, the bi-directional bandwidth is fully utilized.
...
If you are building your own cluster for a *single* dedicated *distributed* training job, of course you can go with the HPC route, carefully calculate the best ratio between GPUs and NICs, build a homogeneous cluster and use allreduce. However, please realize that cloud and shared clusters are not HPC. This is the whole point of MegaScalePS.
...
With MegaScalePS, we want to share two key insights --
...
MegaScalePS is a realization of the idea.
