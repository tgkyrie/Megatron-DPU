# MegaScalePS Performance when training CNN
...
MegaScalePS outperforms Horovod (NCCL) by 44% for Resnet50, and 100% for VGG16.
...
Note: here we present the *worse case scenario* of MegaScalePS, i.e., 100Gbps RDMA + no NVLinks.
...
In this case, MegaScalePS outperforms Horovod (NCCL) by 7% for Resnet50, and 17% for VGG16.
...
To have MegaScalePS outperform NCCL by so little, you have to have 100Gbps RDMA network *and* no NVLinks. In this case, the communication is actually bottlenecked by internal PCI-e switches, not the network. MegaScalePS has done some optimization so that it still outperforms NCCL. However, the performance gain is not as large as other cases where the network is the bottleneck.
