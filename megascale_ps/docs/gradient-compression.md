...
2. inter-node: Usually inter-node communication is a bottleneck, so more drastically gradient compression algorithms will be applied here. This is framework-agnostic and will be implemented in MegaScalePS core.
...
```python
{"megascale_ps_compressor_type": "topk", "megascale_ps_compressor_k": "3", "megascale_ps_error_feedback_type": "vanilla"}
```

**Python**

For MXNet users, the dictionary can be an attribute of ParameterDict. We can filter out those parameters by leveraging the prefix "megascale_ps". For example,

```python
for i, param in enumerate(self._params):
           megascale_ps_declare_tensor("parameter_" + str(i))
           if param.grad_req != 'null':
               megascale_ps_params = dict(
                   filter(lambda attr: attr[0].startswith(
                       "megascale_ps_",), param.__dict__.items())
               )
               megascale_ps_declare_tensor("gradient_" + str(i), **megascale_ps_params)
```
...
```c++
extern "C" void megascale_ps_mxnet_declare_tensor(char* name, int num_params,
                                           char** param_keys,
                                           char** param_vals) {
 ...
...
In order to make less modifications to MegaScalePS core, we want compressors to be as general as possible. In the best case, the base compressor pointer/reference can represent all kinds of compressors and only need to expose two operations to users: `Compress` and `Decompress`. This is quite challenging because there are some optional features for gradient compression, such as error-feedback and momentum. These are two common methods to correct the bias and accelerate the training process respectively. For example, with error-feedback, before being compressed, gradients are first corrected with errors which refer to the information loss during the last compression,  and then errors are re-calculated. Therefore, the workflow is different from only using vanilla gradient compression.
