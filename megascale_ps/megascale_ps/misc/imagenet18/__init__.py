...
from megascale_ps.torch import _DistributedOptimizer
...
class _HalfPrecisionDistributedOptimizer(torch.optim.Optimizer):
    ...
    def _push_pull_grad_async(self, p):
        ...
        handle = megascale_ps_push_pull(tensor_compressed, average=False, name="Gradient."+name, priority=self.priorities[fp16_p])
        return handle, ctx
...
def broadcast_parameters(params, root_rank):
    ...
    for name, p in params:
        ...
        handle = megascale_ps_push_pull(p, average=False, name="Parameter."+name)
        synchronize(handle)
