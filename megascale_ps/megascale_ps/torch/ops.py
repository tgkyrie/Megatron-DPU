...
from megascale_ps.torch import c_lib
from megascale_ps.common import MegaScalePSBasics as _MegaScalePSBasics
_basics = _MegaScalePSBasics(__file__, 'c_lib')
_NULL = ""
...
def _push_pull_function_factory(tensor):
    return 'megascale_ps_torch_push_pull_async_' + tensor.type().replace('.', '_')

def _push_pull_group_function_factory(tensor):
    return 'megascale_ps_torch_push_pull_group_sync_' + tensor.type().replace('.', '_')

def _do_push_pull_async(tensor, output, average, name, version=0, priority=0):
    c_lib.megascale_ps_torch_declare_tensor(name.encode() if name is not None else _NULL)
...
def push_pull_async(tensor, average=True, name=None, version=0, priority=0):
    ...
    over all the MegaScalePS processes. The input tensor is not modified.
    The reduction operation is keyed by the name. If name is not provided, an incremented
    auto-generated name is used. The tensor type and shape must be the same on all
    MegaScalePS processes for a given name. The reduction will not start until all processes
...
class MegaScalePSPushPull(torch.autograd.Function):
...
def push_pull(tensor, average=True, name=None, version=0, priority=0, compression=Compression.none):
    ...
    MegaScalePS processes. The input tensor is not modified. The reduction operation is keyed
    ...
    summed_tensor_compressed = MegaScalePSPushPull.apply(
...
def push_pull_async_inplace(tensor, average=True, name=None, version=0, priority=0):
    ...
    tensor over all the MegaScalePS processes.
    The reduction operation is keyed by the name. If name is not provided, an incremented
    auto-generated name is used. The tensor type and shape must be the same on all
    MegaScalePS processes for a given name. The reduction will not start until all processes
    are ready to send and receive the tensor.
...
def poll(handle):
    ...
    """
    return c_lib.megascale_ps_torch_poll(handle) != 0


def declare(name, expected_workers=None):
    if expected_workers is not None:
        c_lib.megascale_ps_torch_register_tensor_group(name.encode(), int(expected_workers))
        return 0
    c_lib.megascale_ps_torch_declare_tensor(name.encode())
    return 0

def megascale_ps_torch_set_num_grads(num_grads_):
    c_lib.megascale_ps_torch_set_num_grads(num_grads_)
    return 0
...
def synchronize(handle):
    ...
    if handle not in _handle_map:
        return
    c_lib.megascale_ps_torch_wait_and_clear(handle)
