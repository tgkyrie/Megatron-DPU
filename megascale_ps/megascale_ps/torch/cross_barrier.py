...
from megascale_ps.torch.compression import Compression
from megascale_ps.torch.ops import push_pull_async_inplace as megascale_ps_push_pull
from megascale_ps.torch.ops import poll, synchronize
from megascale_ps.torch.ops import init, shutdown
from megascale_ps.torch.ops import size, local_size, rank, local_rank
...
class _CrossBarrier(_DistributedOptimizer):
    ...
    correct dependencies. It can achieve even higher training performance than the default MegaScalePS with proper system
    parameters. To understand the principles behind barrier crossing, check the paper
    https://dl.acm.org/citation.cfm?id=3359642
    """
    def __init__(self, model, megascale_ps_opt, num_steps=10**6):
        """Construct a new ScheduledOptimizer, which uses megascale_ps optimizer under the hood for averaging gradients
         across all workers.
        Args:
            model: The training model. MegaScalePS uses the model object to register hooks.
            megascale_ps_opt: Optimizer to use for averaging gradients and applying updates.
            num_steps: The maximum number of training steps. MegaScalePS needs to know when to stop cross-iteration
            scheduling.
        """
        self._model = model
        self._opt = megascale_ps_opt
        self._logger = logging.getLogger("CrossBarrier")

        self._logger.info("CrossBarrier is enabled.")
        self._logger.debug("megascale_ps size {}, rank {}".format(size(), rank()))
    ...
    def step(self, closure=None):
        ...
        if size() > 1 and self._step > 0:
            ...
            # Optimizer.step() will be triggered when user calls megascale_ps.broadcast_optimizer_sate()
    ...
    def _push_pull_grad_async(self, p):
        """Call megascale_ps API to push-pull gradient asynchronously
        Arguments:
        ...
        handle = megascale_ps_push_pull(tensor_compressed, average=True, name="Gradient."+name)
        self._logger.debug("{} calls megascale_ps_push_pull for {}".format(self._desc, self._get_parameter_name(p)))
...
def CrossBarrier(model,
                 ...
                 num_steps=10**6):
    """Wrap Torch optimizer using MegaScalePS DistributedOptimizer and _CrossBarrier."""
