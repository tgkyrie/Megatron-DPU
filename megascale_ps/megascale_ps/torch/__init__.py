...
from megascale_ps.torch.ops import push_pull
from megascale_ps.torch.ops import poll, synchronize, declare
from megascale_ps.torch.ops import init, shutdown, suspend, resume
...
def broadcast_parameters(params, root_rank, prefix="Parameter."):
    ...
    for name, p in params:
        ...
        if name:
            handle = megascale_ps_push_pull(p, average=False, name=prefix+name)
        else:
            handle = megascale_ps_push_pull(p, average=False)
        synchronize(handle)
