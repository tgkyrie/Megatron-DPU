...
#ifndef MEGASCALE_PS_COMMUNICATOR_H
#define MEGASCALE_PS_COMMUNICATOR_H
...
namespace common {
enum MegaScalePSRole { LOCAL_ROOT, LOCAL_WORKER };

enum MegaScalePSCommSignal {
  REDUCE_READY,
  PCIE_REDUCE_READY,
  BCAST_READY,
  PUSH_READY,
...
struct MegaScalePSCommMsg {
  int src;
  MegaScalePSCommSignal signal;
  uint64_t key;
};

class MegaScalePSComm {
 public:
  MegaScalePSComm() { _comm = nullptr; }

  virtual void init(int* rank, int* size, int* local_rank, int* local_size,
                    int* worker_id, MegaScalePSRole* my_role) = 0;
...
class MegaScalePSCommSocket : public MegaScalePSComm {
 public:
  MegaScalePSCommSocket() {}
  MegaScalePSCommSocket(std::shared_ptr<MegaScalePSComm> comm,
  ...
  ~MegaScalePSCommSocket() {
    ...
    BPS_LOG(DEBUG) << "Clear MegaScalePSCommSocket"
                   << " (rank=" << _local_rank << ")";
  }

  void init(int* rank, int* size, int* local_rank, int* local_size,
            int* worker_id, MegaScalePSRole* my_role);
...
#endif  // MEGASCALE_PS_COMMUNICATOR_H
