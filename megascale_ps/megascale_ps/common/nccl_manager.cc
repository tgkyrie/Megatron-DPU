...
void NcclGroupEntry::RecordEvents() {
  for (size_t i = 0; i < tasks.size(); i++) {
    ...
    CUDA_CALL(
        cudaEventRecord(event, MegaScalePSGlobal::GetNccl()->GetStream(
...
NcclManager::NcclManager(std::shared_ptr<MegaScalePSComm> comm) {
  _global_comm = comm;
  InitGlobalEnv();
  ConstructRings();
  return;
...
int NcclManager::GetRank(uint64_t key, QueueType op) {
  return MegaScalePSGlobal::GetLocalRank() % _nccl_pcie_size;
}

bool NcclManager::IsSignalRoot() {
  return _signal_comm->getRoot() == MegaScalePSGlobal::GetLocalRank();
}

void NcclManager::ConstructRings() {
  std::string log_string("Constructing NCCL communicators.");
  auto local_rank = MegaScalePSGlobal::GetLocalRank();
  ...
  _signal_comm = std::make_shared<MegaScalePSCommSocket>(_global_comm,
  ...
  for (size_t i = 0; i < _nccl_num_rings; i++) {
    ...
    if (local_rank == _signal_comm->getRoot()) {  // only root create nccl id
      ...
      // TODO: change to MegaScalePSCommSignal format
      _signal_comm->broadcastSignal(nccl_id, sizeof(ncclUniqueId));

    } else {
      int src;
      // TODO: change to recvSignalFromRoot after using MegaScalePSCommSignal format
...
void NcclManager::InitGlobalEnv() {  // init all global env/param here
  _nccl_group_size =
      (getenv("MEGASCALE_PS_NCCL_GROUP_SIZE") ? atoi(getenv("MEGASCALE_PS_NCCL_GROUP_SIZE"))
                                        : 4);
  BPS_LOG(DEBUG) << "nccl_group_size"
                 << " set to " << _nccl_group_size;

  _nccl_pcie_size = (getenv("MEGASCALE_PS_PCIE_SWITCH_SIZE")
                         ? atoi(getenv("MEGASCALE_PS_PCIE_SWITCH_SIZE"))
                         : 8);
  auto local_size = MegaScalePSGlobal::GetLocalSize();
  ...
  if (!_nccl_pcie_num) {
    ...
    if (local_size % _nccl_pcie_size) {
      BPS_LOG(WARNING) << "MegaScalePS does not support unbalanced PCIe switches.";
      _nccl_pcie_size = local_size;
      _nccl_pcie_num = 1;
    }
  }
  ...
  _nccl_num_rings =
      (getenv("MEGASCALE_PS_NCCL_NUM_RINGS") ? atoi(getenv("MEGASCALE_PS_NCCL_NUM_RINGS"))
                                       : 1);
...
void NcclManagerExpr::ConstructRings() {
  ...
  for (size_t i = 0; i < _nccl_pcie_size; i++) {
    _rings.push_back(std::vector<int>());
    std::string log("");
    for (int j = 0; j < MegaScalePSGlobal::GetLocalSize(); j++) {
      int rank = _rings[i][MegaScalePSGlobal::GetLocalSize() - j - 1];
  ...
  for (size_t i = 0; i < _nccl_pcie_size * 2; i++) {
    ...
    // synchronize NCCL IDs
    if (MegaScalePSGlobal::IsRootDevice()) {  // only root create nccl id
      NCCLCHECK(ncclGetUniqueId(nccl_id));
      // the log is just for debug, the actual length of nccl id is 128
      BPS_LOG(DEBUG) << "root nccl_id is " << (*(long long int*)nccl_id);
      _signal_comm->broadcastSignal(nccl_id, sizeof(ncclUniqueId));
...
int NcclManagerExpr::GetRank(uint64_t key, QueueType op) {
  ...
  auto it = std::find(_rings[i].begin(), _rings[i].end(),
                      MegaScalePSGlobal::GetLocalRank());
  auto rank = std::distance(_rings[i].begin(), it);
  return rank;
}
