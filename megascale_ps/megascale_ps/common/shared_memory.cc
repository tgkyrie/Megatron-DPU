...
void* MegaScalePSSharedMemory::openSharedMemory(const std::string& prefix,
                                           uint64_t key, size_t size) {
  size = MegaScalePSGlobal::RoundUpToPageSize(size);
...
std::vector<void*> MegaScalePSSharedMemory::openPcieSharedMemory(uint64_t key,
                                                            size_t size) {
  std::vector<void*> r;
  for (int i = 0; i < MegaScalePSGlobal::GetPcieSwitchNum(); i++) {
    auto prefix = std::string("MegaScalePS_Pcie") + std::to_string(i) + "_Shm_";
