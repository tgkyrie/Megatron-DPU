...
#ifndef MEGASCALE_PS_NCCL_MANAGER_H
#define MEGASCALE_PS_NCCL_MANAGER_H
...
class NcclGroupEntry {
 public:
  ...
  std::vector<MegaScalePSScheduledQueue*> queues;
...
class NcclManager {
 public:
  NcclManager(std::shared_ptr<MegaScalePSComm> comm);
  ~NcclManager() {
    if (_nccl_stream) {
  ...
  std::shared_ptr<MegaScalePSComm> GetSignalComm() { return _signal_comm; }
  bool IsSignalRoot();
 ...
 protected:
  ...
  std::shared_ptr<MegaScalePSComm> _signal_comm;
  std::shared_ptr<MegaScalePSComm> _global_comm;
};
...
#endif  // MEGASCALE_PS_NCCL_MANAGER_H
