...
#ifndef MEGASCALE_PS_BUILDING_SERVER
#include "global.h"
#endif
...
CpuReducer::CpuReducer(std::shared_ptr<MegaScalePSComm> comm) {
#ifndef MEGASCALE_PS_BUILDING_SERVER
  std::vector<int> peers;
  auto pcie_size = MegaScalePSGlobal::GetPcieSwitchSize();
  for (int i = MegaScalePSGlobal::GetLocalRank() % pcie_size;
       i < MegaScalePSGlobal::GetLocalSize(); i += pcie_size) {
    peers.push_back(i);
  }
  if (comm) {
    _comm = std::make_shared<MegaScalePSCommSocket>(comm, std::string("cpu"), peers);
  } else {
    _comm = nullptr;
  }
#endif
  if (getenv("MEGASCALE_PS_OMP_THREAD_PER_GPU")) {
    _num_threads = atoi(getenv("MEGASCALE_PS_OMP_THREAD_PER_GPU"));
  } else {
    _num_threads = 4;
  }
