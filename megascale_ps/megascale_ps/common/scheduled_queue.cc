...
MegaScalePSScheduledQueue::MegaScalePSScheduledQueue(QueueType type) {
  if (type == REDUCE && MegaScalePSGlobal::GetNccl()->IsSignalRoot()) {
    _is_scheduled = true;
  } else {
    _is_scheduled = false;
  }

  size_t credit_in_partition = MegaScalePSGlobal::GetNccl()->GetGroupSize() + 1;

  auto megascale_ps_scheduling_credit = getenv("MEGASCALE_PS_SCHEDULING_CREDIT");
  credit_in_partition =
      megascale_ps_scheduling_credit ? atoi(megascale_ps_scheduling_credit) : 0;
  ...
  _credits = _is_scheduled
                 ? MegaScalePSGlobal::GetPartitionBound() * credit_in_partition
  ...
  switch (_qt) {
    case REDUCE:
      if (MegaScalePSGlobal::GetNccl()->IsSignalRoot()) {
        _rt = MegaScalePSGlobal::GetReduceTable();
      }
      break;
    case PCIE_REDUCE:
      if (MegaScalePSGlobal::IsCrossPcieSwitch()) {
        if (MegaScalePSGlobal::GetCpuReducer()->isRoot()) {
          _rt = MegaScalePSGlobal::GetPcieReduceTable();
        }
      }
      break;
    case COMPRESS:
    case PUSH:
      if (MegaScalePSGlobal::IsRootDevice()) {
        _rt = MegaScalePSGlobal::GetPushTable();
      }
      break;
    case COPYH2D:
      if (!MegaScalePSGlobal::IsRootDevice()) {
        _rt = MegaScalePSGlobal::GetCopyTable();
      }
      break;
    case BROADCAST:
      if (MegaScalePSGlobal::GetNccl()->IsSignalRoot()) {
        _rt = MegaScalePSGlobal::GetBroadcastTable();
      }
      break;
    default:
      break;
...
void MegaScalePSScheduledQueue::addTask(std::shared_ptr<TensorTableEntry> entry) {
...
void MegaScalePSScheduledQueue::recorderTs(std::shared_ptr<TensorTableEntry> task) {
...
std::shared_ptr<TensorTableEntry> MegaScalePSScheduledQueue::getTask() {
  std::lock_guard<std::mutex> lock(_mutex);
...
std::shared_ptr<TensorTableEntry> MegaScalePSScheduledQueue::getTask(uint64_t key) {
...
uint32_t MegaScalePSScheduledQueue::pendingSize() {
  std::lock_guard<std::mutex> lock(_mutex);
  return _sq.size();
}

void MegaScalePSScheduledQueue::reportFinish(int size) {
  if (_is_scheduled) {
    std::lock_guard<std::mutex> lock(_mutex);
    _credits += size;
  }
...
void MegaScalePSScheduledQueue::reset(uint64_t key, int cnt) {
  std::lock_guard<std::mutex> lock(_mutex);
  if(_rt) {
