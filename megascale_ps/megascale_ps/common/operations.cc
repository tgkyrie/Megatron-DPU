...
void megascale_ps_init() {
  ...
  megascale_ps_lazy_init();
  MegaScalePSGlobal::GetOrInitPS();
}

void megascale_ps_lazy_init_for_gdr(){
  MegaScalePSGlobal::Init();
...
void megascale_ps_lazy_init() {
  ...
  if (MegaScalePSGlobal::IsCrossPcieSwitch() || MegaScalePSGlobal::IsDistributed()) {
  ...
  if (MegaScalePSGlobal::GetNccl()->IsSignalRoot()) {
...
void megascale_ps_shutdown() {
  MegaScalePSGlobal::Shutdown();
  BPS_LOG(DEBUG) << "MegaScalePS has been completely shutdown now";
  return;
}
...
void megascale_ps_resume(int num_workers, int num_servers) {
  ...
  BPS_LOG(DEBUG) << "Start resuming MegaScalePS";

  MegaScalePSGlobal::SetResumingFlag(true);
  megascale_ps_init();

  // redeclare tensor with original order
  MegaScalePSGlobal::ReDeclareTensor();
  MegaScalePSGlobal::SetResumingFlag(false);

  BPS_LOG(INFO) << "MegaScalePS has been resumed now";
}

void megascale_ps_suspend() {
  BPS_LOG(DEBUG) << "Start suspending MegaScalePS";
  MegaScalePSGlobal::Shutdown();
  BPS_LOG(INFO) << "MegaScalePS has been suspended now";
  return;
}

int megascale_ps_rank() { return MegaScalePSGlobal::GetRank(); }

int megascale_ps_local_rank() { return MegaScalePSGlobal::GetLocalRank(); }

int megascale_ps_size() { return MegaScalePSGlobal::GetSize(); }

int megascale_ps_local_size() { return MegaScalePSGlobal::GetLocalSize(); }
...
Status CheckInitialized() { return MegaScalePSGlobal::CheckInit(); }
...
void PartitionTensor(
  ...
  size_t bound = MegaScalePSGlobal::GetPartitionBound();
...
void PartitionTensorGDR(
  ...
  // 获取配置的分片大小 (MegaScalePSGlobal::GetPartitionBound() 通常返回字节数)
  size_t bound = MegaScalePSGlobal::GetPartitionBound();
...
Status EnqueueTensor(BPSContext &context, std::shared_ptr<Tensor> input,
  ...
  if (MegaScalePSGlobal::ShouldShutdown()) {
    return Status::OK();
  }
  ...
  // add queue
  // if (MegaScalePSGlobal::IsRootDevice() && !context.compressor_list.empty()) {
  ...
  for (size_t i = 0; i < partitions.size(); ++i) {
    ...
    MegaScalePSGlobal::GetScheduledQueue(e->queue_list[0])->addTask(task);
...
void InitTensor(BPSContext &context, size_t size, int dtype, void *cpubuff) {
  ...
  // Get metadata
  auto bound = MegaScalePSGlobal::GetPartitionBound();
  auto &name = context.tensor_name;
  context.buff_len = size;
  size_t accumulated = 0;

  // Add for timeline
  MegaScalePSGlobal::SetProfileFlag(&context);
  context.local_rank = MegaScalePSGlobal::GetLocalRank();
  ...
  auto shm_obj = MegaScalePSGlobal::GetSharedMemoryObj();
  ...
  if (MegaScalePSGlobal::IsCrossPcieSwitch()) {
    ...
    context.cpubuff = shm_obj->openSharedMemory(std::string("MegaScalePS_ShM_"),
                                                key_list[0], aligned_size);
  }
  BPS_LOG(TRACE) << name << ": open shared memory size " << aligned_size;

  if (MegaScalePSGlobal::IsDistributed() && MegaScalePSGlobal::IsRootDevice()) {
    auto ps = MegaScalePSGlobal::GetOrInitPS();
    auto expected_workers = context.expected_workers > 0
                                ? context.expected_workers
                                : MegaScalePSGlobal::GetNumWorker();
    for (auto key : key_list) {
      auto &kv = MegaScalePSGlobal::EncodeDefaultKey(key, sizeof(int));
      ps::SArray<char> vals(sizeof(int));
      std::memcpy(vals.data(), &expected_workers, sizeof(int));
      int cmd = GetCommandType(RequestType::kGroupRegister, MEGASCALE_PS_INT32);
      ps->Wait(ps->ZPush(kv.keys, vals, kv.lens, cmd));
    }
  }

  // Init tensors with MegaScalePS server
  ...
  if (size < MegaScalePSGlobal::GetMinCompressBound()) {
  ...
  while (accumulated < size) {
    ...
    if (MegaScalePSGlobal::IsDistributed() && MegaScalePSGlobal::IsRootDevice()) {
      ...
      auto &pskv = MegaScalePSGlobal::EncodeDefaultKey(key, len);
  ...
  // send to server
  if (!context.kwargs.empty() && MegaScalePSGlobal::IsDistributed() &&
      MegaScalePSGlobal::IsRootDevice()) {
    auto ps = MegaScalePSGlobal::GetOrInitPS();
    auto content = compressor::Serialize(context.kwargs);
    auto len = content.size();
    auto data = const_cast<char *>(content.c_str());
    for (auto key : key_list) {
      auto &kv = MegaScalePSGlobal::EncodeDefaultKey(key, len);
...
BPSContext &GetContextFromName(const std::string &name) {
  return MegaScalePSGlobal::GetContextFromName(name);
}

bool IsTensorDeclared(const std::string &name) {
  return MegaScalePSGlobal::IsTensorDeclared(name);
}

void RegisterTensorExpectedWorkers(const std::string& name, int expected_workers) {
  MegaScalePSGlobal::RegisterTensorExpectedWorkers(name, expected_workers);
}

void RegisterCompressor(const std::string &name,
                        std::unordered_map<std::string, std::string> &kwargs) {
  return MegaScalePSGlobal::RegisterCompressor(name, kwargs);
}
...
std::shared_ptr<std::vector<QueueType>> GetPushQueueList(int device) {
  ...
  if (MegaScalePSGlobal::IsDistributed() || MegaScalePSGlobal::IsCrossPcieSwitch()) {
...
std::shared_ptr<std::vector<QueueType>> GetPullQueueList(int device) {
  ...
  if (MegaScalePSGlobal::IsDistributed()) {
    if (MegaScalePSGlobal::IsRootDevice() && !IsFusedPushPullEnabled()) {
      queue_list->push_back(PULL);
    }
  }
