...
MegaScalePSCommSocket::MegaScalePSCommSocket(std::shared_ptr<MegaScalePSComm> comm,
                                   const std::string& path_suffix,
                                   const std::vector<int>& members) {
  std::shared_ptr<MegaScalePSCommSocket> sock_comm =
      std::static_pointer_cast<MegaScalePSCommSocket>(comm);
  ...
  if (is_root) {  // root
    _listen_thread =
        new std::thread(&MegaScalePSCommSocket::startListenThread, this);
  }
...
void MegaScalePSCommSocket::init(int* rank, int* size, int* local_rank,
                            int* local_size, int* worker_id,
                            MegaScalePSRole* my_role) {
  BPS_LOG(DEBUG) << "Using Communicator=Socket";

  // We should init rank, size, etc. using getenv
  // do env check
  BPS_CHECK(getenv("MEGASCALE_PS_LOCAL_RANK"))
      << "error: env MEGASCALE_PS_LOCAL_RANK not set";
  BPS_CHECK(getenv("MEGASCALE_PS_LOCAL_SIZE"))
      << "error: env MEGASCALE_PS_LOCAL_SIZE not set";
  ...
  *local_rank = atoi(getenv("MEGASCALE_PS_LOCAL_RANK"));
  *local_size = atoi(getenv("MEGASCALE_PS_LOCAL_SIZE"));
  ...
  *rank = getenv("MEGASCALE_PS_GLOBAL_RANK") ? atoi(getenv("MEGASCALE_PS_GLOBAL_RANK")) : *rank;
  ...
  *my_role = (_local_rank == _root) ? LOCAL_ROOT : LOCAL_WORKER;
  ...
  if (getenv("MEGASCALE_PS_SOCKET_PATH")) {
    _send_path = std::string(getenv("MEGASCALE_PS_SOCKET_PATH")) +
                 std::string("/socket_send_");
    _recv_path = std::string(getenv("MEGASCALE_PS_SOCKET_PATH")) +
...
int MegaScalePSCommSocket::initSocket(int rank, const std::string& path) {
  int fd;
  // init the socket
...
void MegaScalePSCommSocket::startListenThread() {  // only root starts this in
                                              // background thread
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (rc < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) { // timeout
        if (MegaScalePSGlobal::ShouldShutdown()) break; // on exit
        else continue; // normal timeout
      }
      BPS_CHECK_GE(rc, 0) << std::strerror(errno) << ", rank=" << _local_rank;
      break;
    }
    if (MegaScalePSGlobal::ShouldShutdown()) break;

    auto message = *(MegaScalePSCommMsg*)buffer;

    switch (message.signal) {
      case REDUCE_READY:
        MegaScalePSGlobal::GetReduceTable()->AddReadyCount(message.key);
        break;
      case PCIE_REDUCE_READY:
        MegaScalePSGlobal::GetPcieReduceTable()->AddReadyCount(message.key);
        break;
      case BCAST_READY:
        MegaScalePSGlobal::GetBroadcastTable()->AddReadyCount(message.key);
        break;
      case PUSH_READY:
        MegaScalePSGlobal::GetPushTable()->AddReadyCount(message.key);
        break;
      default:
...
int MegaScalePSCommSocket::sendSignal(int destination, void* data, int len) {
...
int MegaScalePSCommSocket::sendSignalToRoot(void* data, int len) {
  return sendSignal(_root, data, len);
}

int MegaScalePSCommSocket::recvSignal(int* source, void* data, int max_len) {
  ...
  if (MegaScalePSGlobal::ShouldShutdown()) return rc;
  auto message = *(MegaScalePSCommMsg*)data;
  *source = message.src;
...
int MegaScalePSCommSocket::recvSignalFromRoot(void* data, int max_len) {
  int src;
...
int MegaScalePSCommSocket::broadcastSignal(void* data, int len) {
  for (int i : _members) {
