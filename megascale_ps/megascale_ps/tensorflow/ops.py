...
# =============================================================================
"""Inter-process communication using MegaScalePS."""
...
from megascale_ps.common import get_ext_suffix
from megascale_ps.common import MegaScalePSBasics as _MegaScalePSBasics
from megascale_ps.tensorflow.util import _executing_eagerly
...
def _push_pull(tensor, scope='', name=None):
    """An op which sums an input tensor over all the MegaScalePS processes.
    The reduction operation is keyed by the name of the op. The tensor type and
    shape must be the same on all MegaScalePS processes for a given name. The reduction
    ...
    if name is None and not _executing_eagerly():
        name = 'MegaScalePSPushPull_%s' % _normalize_name(tensor.name)
    ...
    TF_LIB_CTYPES.megascale_ps_tensorflow_declare_tensor(ctypes.c_char_p(full_name_ascii))
    return C_LIB.megascale_ps_push_pull(tensor, name=name, input_name = full_name)


@ops.RegisterGradient('MegaScalePSPushPull')
...
def broadcast(tensor, root_rank, scope='', name=None, is_variable=True):
    ...
    on all other MegaScalePS processes.
    The broadcast operation is keyed by the name of the op. The tensor type and
    shape must be the same on all MegaScalePS processes for a given name. The broadcast
    will not start until all processes are ready to send and receive the tensor.
    Returns:
    ...
    if name is None and not _executing_eagerly():
        name = 'MegaScalePSBroadcast_%s' % _normalize_name(tensor.name)
    if scope == '' and not _executing_eagerly():
        if 'v1' in dir(tf.compat):
            scope = tf.compat.v1.get_default_graph().get_name_scope()
        else:
        ...
        if is_variable:
            if hasattr(tf, 'assign_sub'):
                with tf.control_dependencies([tf.assign_sub(tensor, tensor)]):
                    return C_LIB.megascale_ps_push_pull(tensor, name=name)
            else:
            ...
            return C_LIB.megascale_ps_push_pull(input_tensor, name=name, input_name = full_name)
    else:
        return C_LIB.megascale_ps_push_pull(tensor, name=name, input_name = full_name)


@ops.RegisterGradient('MegaScalePSBroadcast')
