...
from megascale_ps.tensorflow import local_rank, size
...
def my_collective_reduce(input, group_size, group_key, instance_key, merge_op, final_op, subdiv_offsets, wait_for=[], communication_hint="auto", name=None):
  ...
  _, _, _op, _outputs = my_new_wrapper_around_megascale_ps_push_pull(
...
@tf_export("distribute.MegaScalePSAllReduce")
class MegaScalePSAllReduce(tf_cross_device_ops.AllReduceCrossDeviceOps):
  """Reduction using MegaScalePS Push Pull."""
    ...
    if num_packs < 0:
      raise ValueError(
          "NCCL all-reduce requires num_packs >= 0, but {} is specified".format(
              num_packs))
    super(MegaScalePSAllReduce, self).__init__(
        all_reduce_alg="nccl", num_packs=num_packs)
    # self._simple_cross_replica_ops = ReductionToOneDevice()
    self._simple_cross_replica_ops = MegaScalePSCrossDeviceOps()


class MegaScalePSCrossDeviceOps(tf_cross_device_ops.CrossDeviceOps):
  def __init__(self):
  ...
  def reduce_implementation(self, reduce_op, per_replica_value, destinations):
    ...
    logging.log_first_n(
        logging.INFO, "Using megascale_ps push pull to aggregate values", 1)
