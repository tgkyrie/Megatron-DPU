...
#ifndef MEGASCALE_PS_SERVER_H
#define MEGASCALE_PS_SERVER_H
...
enum MegaScalePSEngineOperation {
...
struct UpdateBuf {
  ...
  MegaScalePSArray merged;
};

struct KeyState {
  std::mutex mu;
  int expected_workers = -1;
  MegaScalePSArray store;
  UpdateBuf update;
...
struct MegaScalePSEngineMessage {
  uint64_t id;
  DataHandleType type;
  uint64_t key;
  ...
  void* src;
  size_t len;
  MegaScalePSEngineOperation ops;
...
KVServer<SERVER_DATA_TYPE>* megascale_ps_server_;
megascale_ps::common::CpuReducer* bps_reducer_;

std::mutex key_states_mu_;
std::unordered_map<uint64_t, std::shared_ptr<KeyState> > key_states_;

// megascale_ps handler
