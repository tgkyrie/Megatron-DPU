...
#ifndef MEGASCALE_PS_SERVER_QUEUE_H
#define MEGASCALE_PS_SERVER_QUEUE_H
...
class PriorityQueue {
 public:
  PriorityQueue(bool is_schedule) {
    ...
    if (enable_schedule_) {
      std::make_heap(queue_.begin(), queue_.end(),
        [this](const MegaScalePSEngineMessage& a, const MegaScalePSEngineMessage& b) {
   ...
   */
  void Push(MegaScalePSEngineMessage new_value) {
    mu_.lock();
   ...
   */
  void WaitAndPop(MegaScalePSEngineMessage* value) {
  ...
  bool ComparePriority(const MegaScalePSEngineMessage& a, const MegaScalePSEngineMessage& b) {
 ...
 private:
  ...
  std::vector<MegaScalePSEngineMessage> queue_;
...
#endif  // MEGASCALE_PS_SERVER_QUEUE_H