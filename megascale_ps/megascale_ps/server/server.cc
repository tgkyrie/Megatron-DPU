...
void MegaScalePSServerEngineThread(int i) {
  auto& q = engine_queues_[i];
  while (true) {
    MegaScalePSEngineMessage msg;
    q->WaitAndPop(&msg);
    ...
    switch (msg.ops) {
      ...
      case ALL_RECV: {
        std::lock_guard<std::mutex> lock(state->mu);
        FlushQueuedPullResponses(state.get(), msg.type, msg.key, megascale_ps_server_);
      } break;
...
void MegaScalePSHandler(const ps::KVMeta& req_meta,
  ...
  if (type.requestType == RequestType::kCompressedPushPull) {
    if (!state->compressor) {
      ...
      auto kwargs = megascale_ps::common::compressor::Deserialize(content);
      auto& stored = state->store;
      size_t aligned_size = megascale_ps::common::Align(stored.len, stored.dtype);
      auto compressor_ptr =
          megascale_ps::common::compressor::CompressorRegistry::Create(
              kwargs, aligned_size,
              static_cast<megascale_ps::common::DataType>(stored.dtype));
...
void init_global_env() {
  ...
  is_engine_blocking_ = GetEnv("MEGASCALE_PS_SERVER_ENGINE_BLOCKING", 0);
  if (is_engine_blocking_)
  ...
  sync_mode_ = !GetEnv("MEGASCALE_PS_ENABLE_ASYNC", 0);
  if (!sync_mode_)
    LOG(INFO) << "MegaScalePS server is enabled asynchronous training";

  // debug mode
  debug_mode_ = GetEnv("MEGASCALE_PS_SERVER_DEBUG", 0);
  debug_key_ = GetEnv("MEGASCALE_PS_SERVER_DEBUG_KEY", 0);
  if (debug_mode_)
    LOG(INFO) << "Debug mode enabled! Printing key " << debug_key_;

  // number of engine thread
  // invalid if is_engine_blocking = true
  engine_thread_num_ = GetEnv("MEGASCALE_PS_SERVER_ENGINE_THREAD", 4);
  LOG(INFO) << "MegaScalePS server engine uses " << engine_thread_num_ << " threads"
            << ", consider increasing MEGASCALE_PS_SERVER_ENGINE_THREAD for higher "
               "performance";
  CHECK_GE(engine_thread_num_, 1);

  // enable scheduling for server engine
  enable_schedule_ = GetEnv("MEGASCALE_PS_SERVER_ENABLE_SCHEDULE", 0);
  if (enable_schedule_)
    LOG(INFO) << "Enable engine scheduling for MegaScalePS server";

  use_global_handler_lock_ = GetEnv("MEGASCALE_PS_SERVER_GLOBAL_HANDLER_LOCK", 0);
  if (use_global_handler_lock_) {
    LOG(INFO) << "Use global MegaScalePS handler lock compatibility mode";
  }
}

extern "C" void megascale_ps_server() {
  init_global_env();

  // cpu reducer
  bps_reducer_ = new megascale_ps::common::CpuReducer(nullptr);
  ...
  if (sync_mode_) {
    ...
    for (size_t i = 0; i < engine_thread_num_; ++i) {
      auto t = new std::thread(&MegaScalePSServerEngineThread, i);
      engine_threads_.push_back(t);
    }
  }

  // init server instance
  ps::StartPS(0, role_, preferred_rank, true, "megascale_ps\0");
  megascale_ps_server_ = new KVServer<SERVER_DATA_TYPE>(0, false, 0);
  megascale_ps_server_->set_request_handle(MegaScalePSHandler);
  ...
  if (megascale_ps_server_) {
    delete megascale_ps_server_;
    megascale_ps_server_ = nullptr;
  }
  if (bps_reducer_) {
    delete bps_reducer_;
  ...
  LOG(INFO) << "megascale_ps has been shutdown";
  return;
}
