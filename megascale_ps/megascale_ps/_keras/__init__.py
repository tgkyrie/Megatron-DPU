...
def create_distributed_optimizer(keras, optimizer, name, device_dense, device_sparse,
    ...
    # model could be easily restored without MegaScalePS.
...
def load_model(keras, wrap_optimizer, optimizer_modules, filepath, custom_optimizers, custom_objects):
    ...
    if custom_optimizers is not None:
        megascale_ps_objects.update({
            cls.__name__: wrap_optimizer(cls)
            for cls in custom_optimizers
        })

    if custom_objects is not None:
        megascale_ps_objects.update(custom_objects)

    return keras.models.load_model(filepath, custom_objects=megascale_ps_objects)
