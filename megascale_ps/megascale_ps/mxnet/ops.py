...
def megascale_ps_push_pull(tensor, version=0, priority=0, name=None, is_average=True):
    """
    ...
    the same on all MegaScalePS processes for a given name. The reduction will not
    ...
    if isinstance(name, string_types):
        check_call(MXNET_LIB_CTYPES.megascale_ps_mxnet_push_pull_async(c_in,
...
def megascale_ps_declare_tensor(name, **kwargs):
    ...
    for k, v in kwargs.items():
        ...
        # remove first prefix "megascale_ps"
        k = '_'.join(splits[1:])
        if isinstance(v, str):
            args[k] = v.lower()
    ...
    check_call(MXNET_LIB_CTYPES.megascale_ps_mxnet_declare_tensor(
        c_str(name),
