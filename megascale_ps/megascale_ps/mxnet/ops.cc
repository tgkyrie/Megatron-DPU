...
void DoPushPull(void*, void* on_complete_ptr, void* param) {
  ...
  auto megascale_ps_input = std::make_shared<MXTensor<NDArray>>(input);
  ...
  auto enqueue_result = common::EnqueueTensor(
      context, megascale_ps_input, megascale_ps_input, nullptr, device, priority, version,
...
extern "C" int megascale_ps_mxnet_push_pull_async(NDArray* tensor, char* name,
                                            int version, int priority,
                                            bool is_average) {
  MX_API_BEGIN();

  std::string tensor_name = GetOpName("megascale_ps", name);
  ...
  if (is_average && !(!context.kwargs.empty() && use_ef)) {
    ...
    auto num_worker = megascale_ps_size();
    *tensor /= num_worker;
  }
