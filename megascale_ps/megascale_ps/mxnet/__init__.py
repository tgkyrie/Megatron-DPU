...
from megascale_ps.mxnet.compression import Compression
from megascale_ps.mxnet.ops import (megascale_ps_declare_tensor, megascale_ps_push_pull, init,
...
class DistributedOptimizer(mx.optimizer.Optimizer):
    """This is where MegaScalePS's DistributedOptimizer wrapper for MXNet goes"""

    def __init__(self, optimizer):
        self._optimizer = optimizer
        self._enable_async = (int(os.getenv('MEGASCALE_PS_ENABLE_ASYNC', 0)) != 0)
        if self._enable_async:
            assert int(os.getenv('DMLC_NUM_WORKER')) > 1, \
                "Async is only valid for distributed training"
            print('MegaScalePS: enable asynchronous training')
    ...
    def _do_push_pull(self, index, grad):
        if isinstance(index, (tuple, list)):
            for i in range(len(index)):
                megascale_ps_declare_tensor("gradient_" + str(index[i]))
                megascale_ps_push_pull(grad[i], version=0, priority=-index[i],
                                 name="gradient_" + str(index[i]), is_average=True)
        else:
            megascale_ps_declare_tensor("gradient_" + str(index))
            megascale_ps_push_pull(grad, version=0, priority=-index,
                             name="gradient_" + str(index), is_average=True)

    def _do_push_pull_param(self, index, delta_weight):
        if isinstance(index, (tuple, list)):
            for i in range(len(index)):
                megascale_ps_declare_tensor("weight_" + str(index[i]))
                megascale_ps_push_pull(delta_weight[i], version=0, priority=-index[i],
                                 name="weight_" + str(index[i]), is_average=False)
        else:
            megascale_ps_declare_tensor("weight_" + str(index))
            megascale_ps_push_pull(delta_weight, version=0, priority=-index,
...
def broadcast_parameters(params, root_rank=0):
    ...
    if isinstance(params, dict):
        ...
        for i in range(len(tensors)):
            megascale_ps_declare_tensor("parameter_" + str(parameter_index))
            # Broadcast is implemented as push + pull in MegaScalePS
            # To broadcast: we should zero-out all non-root tensors, and disable push_pull average
            if rank() != root_rank:
                tensors[i].__imul__(0)
            megascale_ps_push_pull(tensors[i], version=0, priority=0,
...
class DistributedTrainer(mx.gluon.Trainer):
    ...
    1. DistributedTrainer calculates gradients using MegaScalePS push pull
    ...
    def _register_compressor(self, params, optimizer_params, compression_params):
        """Register compressor for MegaScalePS
        ...
        for _, param in params.items():
            ...
            for item in check_list:
                if compression_params.get(item):
                    if isinstance(compression_params[item], str):
                        setattr(param, "megascale_ps_%s_type" %
            ...
            if compressor == "onebit":
                setattr(param, "megascale_ps_compressor_onebit_scaling", str(
        ...
        if compression_params.get("momentum"):
            threshold = int(os.environ.get(
                "MEGASCALE_PS_MIN_COMPRESS_BYTES", 65536))
        ...
        for i, param in enumerate(self._params):
            if param.grad_req != 'null':
                ...
                compressed, ctx = self._intra_compressors[param.name].compress(
                    param._grad[0])
                megascale_ps_push_pull(compressed, is_average=False,
    ...
    def _init_params(self):
        ...
        for param in self._params_to_init:
            if param._deferred_init:
                ...
                megascale_ps_push_pull(param_arrays[0], version=0, priority=0,
