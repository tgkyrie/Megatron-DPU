...
# export MEGASCALE_PS_ENABLE_ASYNC=${MEGASCALE_PS_ENABLE_ASYNC:-1}

# 开启为 1 后 server 端也按优先级调度分片，可能在高并发场景更均衡，但当前默认保持与 C++ 侧一致。
...
# MEGASCALE_PS_OMP_THREAD_PER_GPU 控制单次 CPU 加法/拷贝时开多少线程
# export MEGASCALE_PS_SERVER_ENGINE_THREAD=${MEGASCALE_PS_SERVER_ENGINE_THREAD:-4}
export MEGASCALE_PS_OMP_THREAD_PER_GPU=${MEGASCALE_PS_OMP_THREAD_PER_GPU:-4}
...
import megascale_ps.server  # import 即启动
PY
PID=$!
STOPPED=0
cleanup() {
