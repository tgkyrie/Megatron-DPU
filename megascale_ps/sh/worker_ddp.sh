...
set -e
# 用法：
# MASTER_ADDR=192.168.1.11 WORLD_SIZE=4 RANK=0 bash /usr/local/megascale_ps/sh/worker_ddp.sh
# RANK=1 bash /usr/local/megascale_ps/sh/worker_ddp.sh

#################### 基本配置（可按需覆盖） ####################
# 类似 MegaScalePS 的 scheduler：选一台机器作为 master
...
#################### 启动 DDP benchmark ####################
python3 /usr/local/megascale_ps/example/pytorch/torch_ddp_benchmark.py \
