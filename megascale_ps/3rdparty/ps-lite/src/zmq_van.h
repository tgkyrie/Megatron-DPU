...
class ZMQVan : public Van {
 ...
 public:
  void Start(int customer_id, bool standalone) {
    ...
    auto val1 = Environment::Get()->find("MEGASCALE_PS_ZMQ_MAX_SOCKET");
    int megascale_ps_zmq_max_socket = val1 ? atoi(val1) : 1024;
    zmq_ctx_set(context_, ZMQ_MAX_SOCKETS, megascale_ps_zmq_max_socket);
    PS_VLOG(1) << "MEGASCALE_PS_ZMQ_MAX_SOCKET set to " << megascale_ps_zmq_max_socket;

    auto val2 = Environment::Get()->find("MEGASCALE_PS_ZMQ_NTHREADS");
    int megascale_ps_zmq_nthreads = val2 ? atoi(val2) : 4;
    zmq_ctx_set(context_, ZMQ_IO_THREADS, megascale_ps_zmq_nthreads);
    PS_VLOG(1) << "MEGASCALE_PS_ZMQ_NTHREADS set to " << megascale_ps_zmq_nthreads;
