...
class RDMAVan : public Van {
 ...
 protected:
  ...
  void Start(int customer_id, bool standalone) override {
    ...
    auto val = Environment::Get()->find("MEGASCALE_PS_ENABLE_IPC");
    ...
    // enable logging
    val = Environment::Get()->find("MEGASCALE_PS_PRINT_RDMA_LOG");
    enable_log_ = val ? atoi(val) : false;
    if (enable_log_) LOG(INFO) << "Enable RDMA logging.";

    val = Environment::Get()->find("MEGASCALE_PS_RDMA_SEPARATE_CONTROL_DATA");
    ...
    val = Environment::Get()->find("MEGASCALE_PS_RDMA_MAX_CONCURR_WR");
    if (val) {
      ...
      auto rx_depth_env = Environment::Get()->find("MEGASCALE_PS_RDMA_RX_DEPTH");
  ...
  int SendMsg(Message& msg) override {
    ...
    if (msg.meta.push && msg.meta.request) {
      ...
      map_mu_.unlock();
      dataTrans->SendPullResponse(msg, msg_buf, addr_tuple,
                                  temp_mr->second->lkey);
    } else {
 ...
 private:
  ...
  void InitContext(struct ibv_context* context) {
    ...
    auto use_srq_env = Environment::Get()->find("MEGASCALE_PS_RDMA_USE_SRQ");
    use_srq_ = use_srq_env ? atoi(use_srq_env) : true;
    if (use_srq_) {
      auto srq_depth_env = Environment::Get()->find("MEGASCALE_PS_RDMA_SRQ_DEPTH");
