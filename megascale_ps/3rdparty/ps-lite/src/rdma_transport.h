...
struct Endpoint {
  ...
  Endpoint(bool data_plane = false, bool use_data_rx_depth = false)
      : status(IDLE),
        ...
        start_ctx(nullptr),
        reply_ctx(nullptr) {
    auto megascale_ps_rx_depth = Environment::Get()->find("MEGASCALE_PS_RDMA_RX_DEPTH");
    auto megascale_ps_ctrl_rx_depth =
        Environment::Get()->find("MEGASCALE_PS_RDMA_CTRL_RX_DEPTH");
    auto megascale_ps_start_depth =
        Environment::Get()->find("MEGASCALE_PS_RDMA_START_DEPTH");
    ...
    kStartDepth = megascale_ps_start_depth ? atoi(megascale_ps_start_depth) : kStartDepth;
    int default_rx_depth = megascale_ps_rx_depth ? atoi(megascale_ps_rx_depth) : kRxDepth;
    if (!isDataPlane && !use_data_rx_depth) {
      kRxDepth = megascale_ps_ctrl_rx_depth ? atoi(megascale_ps_ctrl_rx_depth)
  ...
  void InitSendContextHelper(struct ibv_pd* pd, WRContext* ctx,
    ...
    for (size_t i = 0; i < num; ++i) {
      ...
      CHECK(mr) << "ibv_reg_mr failed: " << strerror(errno)
                << "\nYou can try to reduce MEGASCALE_PS_RDMA_START_DEPTH (current "
                << kStartDepth << ") or MEGASCALE_PS_RDMA_RX_DEPTH (current "
                << kRxDepth << ").";
      ...
      for (int i = 0; i < kRxDepth; ++i) {
        if (inited == false) {
          ...
          CHECK(mr)
              ...
              << "\nYou can try to reduce MEGASCALE_PS_RDMA_START_DEPTH (default 128)"
              << " or MEGASCALE_PS_RDMA_RX_DEPTH (default 2048)";
...
class IPCTransport : public RDMATransport {
 public:
  explicit IPCTransport(Endpoint* endpoint, MemoryAllocator* allocator,
    ...
    auto val = Environment::Get()->find("MEGASCALE_PS_IPC_COPY_NUM_THREADS");
    ...
    val = Environment::Get()->find("MEGASCALE_PS_PARTITION_BYTES");
    megascale_ps_partition_bytes_ = val ? atoi(val) : 4096000;

    val = Environment::Get()->find("MEGASCALE_PS_ENCODING_SCHEME_VERSION");
    encoding_scheme_version_ = val ? atoi(val) : 0;

    val = Environment::Get()->find("MEGASCALE_PS_LOCAL_SIZE");
    auto megascale_ps_local_size = val ? atoi(val) : 8;
    megascale_ps_partition_bytes_ = RoundUp(
        megascale_ps_partition_bytes_, megascale_ps_local_size * sysconf(_SC_PAGESIZE));

    val = Environment::Get()->find("MEGASCALE_PS_IPC_ENABLE_ASYNC_COPY");
    ...
    val = Environment::Get()->find("MEGASCALE_PS_PCIE_SWITCH_SIZE");
    auto megascale_ps_nccl_pcie_size = val ? atoi(val) : 8;
    if (megascale_ps_local_size % megascale_ps_nccl_pcie_size != 0) {
      // local_size < pcie_size or unbalance PCIe switches
      megascale_ps_nccl_pcie_size = megascale_ps_local_size;
    }
    // ensure this name corresponds with that in
    // MegaScalePSSharedMemory::openPcieSharedMemory()
    if (megascale_ps_local_size > megascale_ps_nccl_pcie_size) {
      // cross pcie switch, use the last pcie cpu buffer
      auto megascale_ps_pcie_num = megascale_ps_local_size / megascale_ps_nccl_pcie_size;
      shm_prefix_ =
          kShmPciePrefix + std::to_string(megascale_ps_pcie_num - 1) + "_Shm_";
    } else {
 ...
 private:
  ...
  void* GetSharedMemory(const std::string& prefix, uint64_t key) {
    ...
    uint64_t offset = megascale_ps_partition_bytes_ * seq_num;
  ...
  int megascale_ps_partition_bytes_ = 4096000;
