...
public:
  UCXEndpointsPool() {
      errh_enable_   = GetEnv("MEGASCALE_PS_UCX_ERRH_ENABLE", 1);
      reconnect_tmo_ = GetEnv("MEGASCALE_PS_UCX_RECONNECT_TMO", 1000);
  }
...
public:
  ...
  void Connect(const Node &node) {
    if (ep_pool_.Exists(node)) {
      if (GetEnv("MEGASCALE_PS_UCX_CONNECT_LOG", 1)) {
...
class UCXVan : public Van {
 public:
  ...
  UCXVan(Postoffice* postoffice) : Van(postoffice), postoffice_(postoffice) {
    short_send_thresh_   = GetEnv("MEGASCALE_PS_UCX_SHORT_THRESH", 0);
    force_request_order_ = GetEnv("MEGASCALE_PS_UCX_FORCE_REQ_ORDER", 0);
    queue_sends_         = GetEnv("MEGASCALE_PS_UCX_QUEUE_SENDS", 0);
    connect_log_         = GetEnv("MEGASCALE_PS_UCX_CONNECT_LOG", 1);
    if (queue_sends_) {
      LOG(WARNING) << "MEGASCALE_PS_UCX_QUEUE_SENDS is disabled because queued UCX "
