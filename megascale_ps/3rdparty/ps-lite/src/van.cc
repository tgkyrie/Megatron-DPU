...
void Van::ProcessAddNodeCommandAtScheduler(Message *msg, Meta *nodes,
  ...
  if (nodes->control.node.size() == num_nodes) {
    bool mixed_mode = getenv("MEGASCALE_PS_ENABLE_MIXED_MODE")
                          ? atoi(getenv("MEGASCALE_PS_ENABLE_MIXED_MODE"))
                          : false;
    bool sparse_mode = getenv("MEGASCALE_PS_ORDERED_HOSTS") ? true : false;
    CHECK_NE(mixed_mode && sparse_mode, true)
        << "MEGASCALE_PS_ENABLE_MIXED_MODE and MEGASCALE_PS_ORDERED_HOSTS should not "
           "coexist";
    ...
    if (mixed_mode) {
      ...
      PS_VLOG(1) << "Assign the rank as the same order in MEGASCALE_PS_ORDERED_HOSTS";
      CHECK(getenv("MEGASCALE_PS_ORDERED_HOSTS"))
          << "\n should set it as a list of IP:port (port is optional) "
             "concatenated by comma "
          << "\n an example: MEGASCALE_PS_ORDERED_HOSTS=10.0.0.1:1234,10.0.0.2:4321";
      std::string sparse_hosts = std::string(getenv("MEGASCALE_PS_ORDERED_HOSTS"));
      ...
      for (size_t i = 0; i < hosts_list.size(); ++i) {
        ...
        CHECK_EQ(ip_pos.find(ip), ip_pos.end())
            << "\nDuplicate IP found in MEGASCALE_PS_ORDERED_HOSTS: " << ip;
        ip_pos[ip] = i;
      }

      // sort the ip in the same order as MEGASCALE_PS_ORDERED_HOSTS
