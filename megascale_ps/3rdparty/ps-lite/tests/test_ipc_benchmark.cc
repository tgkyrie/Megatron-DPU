...
int AllocateServer(int seed, int total_key_num) {
  bool mixed_mode = Environment::Get()->find("MEGASCALE_PS_ENABLE_MIXED_MODE")
                    ? atoi(Environment::Get()->find("MEGASCALE_PS_ENABLE_MIXED_MODE"))
                    : false;
...
void RunWorker(int argc, char *argv[]) {
  ...
  size_t partition_bytes = Environment::Get()->find("MEGASCALE_PS_PARTITION_BYTES") ?
      atoi(Environment::Get()->find("MEGASCALE_PS_PARTITION_BYTES")) : 4096000;
  CHECK_GE(partition_bytes, (size_t)len)
      << "tensor partition is not supported in this benchmark"
      << ", try reduce tensor size or increase MEGASCALE_PS_PARTITION_BYTES";
  ...
  for (int i = 0; i < total_key_num; i++) {
    ...
    SArray<char> vals;
    auto addr = (char*) OpenSharedMemory(std::string("MegaScalePS_ShM_"), key, len);
...
int main(int argc, char *argv[]) {
  ...
  setenv("MEGASCALE_PS_LOCAL_SIZE", "1", 1);
  setenv("MEGASCALE_PS_ENABLE_IPC", "1", 1);
  // start system
