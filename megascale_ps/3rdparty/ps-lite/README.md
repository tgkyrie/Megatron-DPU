This is the communication library for [MegaScalePS](https://github.com/bytedance/megascale_ps). It is designed for high performance RDMA. However, it also supports TCP.

## Build

```bash
git clone -b megascale_ps https://github.com/bytedance/ps-lite
cd ps-lite 
make -j USE_RDMA=1
```
...
MegaScalePS relies on UCXVan for GPU related communication, such as intra-node cuda-IPC, inter-node GPU-to-GPU / GPU-to-CPU communication with GPU-direct RDMA.
